  ...
  <div class="admin-logistics">
    ...
    <div class="filters">
      ...
      <select v-model="q.status">
        ...
        <option value="material">采发包</option>
      </select>
    ...
    <table>
      <thead>
        <tr>
          <th>ID</th><th>订单号</th><th>用户ID</th><th>用户名</th><th>SKU</th><th>数量</th><th>状态</th><th>备注</th><th>操作</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="r in records" :key="r.id">
          ...
          <td>
            ...
            <button @click="confirmIssue(r)">确认下发</button>
...
export default {
  ...
  data() {
    return {
      q: { username: '', userId: '', orderNo: '', status: 'material' },
  ...
  methods: {
    async load() {
      ...
      const res = await axios.get('/admin/logistics/pack/list', { params })
      if (res && res.code === 200) {
        const p = res.data || {}
    ...
    async save() { await axios.put(`/admin/logistics/pack/${this.form.id}`, this.form); this.show=false; this.load() },
    async remove(r) { await axios.delete(`/admin/logistics/pack/${r.id}`); this.load() },
    async confirmIssue(r) { await axios.post(`/admin/logistics/pack/${r.id}/confirm-issue`, { remark: '确认下发' }); this.load() },
    exportCsv() { window.location.href = '/api/admin/logistics/pack/export' }
  }
}
</script>
<style scoped>
...
.drawer-body { width: 360px; background: #fff; padding: 16px; overflow-y: auto; }

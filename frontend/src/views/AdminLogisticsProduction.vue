  ...
  <div class="admin-logistics">
    ...
    <div class="filters">
      ...
      <select v-model="q.status">
        ...
        <option value="producing">绣制中</option>
      </select>
      ...
      <tbody>
        <tr v-for="r in records" :key="r.id">
          ...
          <td>
            ...
            <button @click="finishProduction(r)">绣制完成</button>
...
export default {
  ...
  methods: {
    ...
    async save() { await axios.put(`/admin/logistics/production/${this.form.id}`, this.form); this.show=false; this.load() },
    async remove(r) { await axios.delete(`/admin/logistics/production/${r.id}`); this.load() },
    async finishProduction(r) { await axios.post(`/admin/logistics/production/${r.id}/finish-production`, { remark: '绣制完成' }); this.load() }
  }
}
</script>
<style scoped>
